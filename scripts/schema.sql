...
-- (CLI 를 쓸 수 있다면 `supabase db push` 가 정석이다. 이 파일은 그 대체 경로다.)
--
-- 생성되는 것
--   테이블 12개 · 뷰 1개(item_effective) · enum 6개
--   RLS 전 테이블 활성 (정책 없음 — 서버가 secret key 로만 접근)
--   스토리지 버킷 2개 (originals · derivatives, 둘 다 비공개)
